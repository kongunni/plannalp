...
//     // 1) 루트 컨테이너
//     const root = document.createElement("div");
//     root.className = "block block-callout"; // getBlockClass("callout")와 일치
//     root.dataset.bid = block.bid;
//     // 설정 반영(색)
//     applyCalloutStyle(root, meta);
...
//     // 이벤트 연결: 전부 useBlockEditor에 있는 기존 함수 재사용
//     if (handleFocus) editable.addEventListener("focus", (e) => handleFocus(e, index));
//     if (handleInputChange) editable.addEventListener("input", (e) => handleInputChange(e, index));
//     if (handleKeyDown) editable.addEventListener("keydown", (e) => handleKeyDown(e, index));
//     if (handleBlur) {
...
//         handleBlur(index, text);
//         });
//     }

//     wrap.appendChild(editable);

//     // 5) ref 등록 (프로젝트 전반에서 bid 기반 참조를 많이 쓰므로 bid 키로 저장)
...
// /**
//  * 콜아웃 블록 내용만 갱신하고 싶을 때(가상 DOM 없이 수동 갱신)
//  * @param {HTMLElement} root .block 요소
//  * @param {string} nextText
//  */
// export function updateCalloutContentDOM(root, nextText) {
//   if (!root) return;
//   const editable = root.querySelector(".editable");
//   if (editable && editable.innerText !== nextText) {
